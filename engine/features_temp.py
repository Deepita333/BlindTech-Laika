...
def searchOnChrome(query):
    """Searches for a given term on Google Chrome."""
    # Extracts the search term from phrases like "search for..." or "google..."
    search_term = ""
    query = query.lower()
    ...
    if search_term:
        speak(f"Searching for {search_term} on Chrome.")
        try:
            kit.search(search_term)
            speak("Here's what I found.")
        except Exception as e:
            speak("Sorry, I couldn't perform the search at the moment.")
            print(f"Error in searchOnChrome: {e}")
    else:
        speak("Sorry, I didn't understand what you want me to search for.")
